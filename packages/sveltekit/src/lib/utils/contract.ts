import { getParsedError } from './getParsedError.js';
import type { AllowedChainIds } from './networks.js';
...
import type { Config, UseReadContractParameters, UseWatchContractEventParameters, UseWriteContractParameters } from 'wagmi';
import { type WriteContractParameters, type WriteContractReturnType, simulateContract } from 'wagmi/actions';
import type { WriteContractVariables } from 'wagmi/query';
// Re-export types from contractTypes.ts for backward compatibility
export type { InheritedFunctions, GenericContract, GenericContractsDeclaration } from './contractTypes.js';

// Export TransactorFuncOptions early to ensure it's available even if module-level code fails
export type TransactorFuncOptions = {
	onBlockConfirmation?: (txnReceipt: TransactionReceipt) => void;
	blockConfirmations?: number;
};

// Now import and execute module-level code
import deployedContractsData from '../contracts/deployedContracts.js';
import externalContractsData from '../contracts/externalContracts.js';
...
// Execute module-level code - wrap in try-catch to handle SSR issues
import type { GenericContractsDeclaration } from './contractTypes.js';

let contractsData: any;
try {
	contractsData = deepMergeContracts(deployedContractsData, externalContractsData);
} catch (error) {
	console.error('[contract.ts] Error merging contracts:', error);
	contractsData = {};
}
...
// TransactorFuncOptions is already exported at the top of the file
