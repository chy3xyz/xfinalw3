...
<div class="flex flex-col gap-4">
  {#if logs && logs.length > 0}
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-border bg-muted/50">
              ...
              <th class="px-3 py-2 text-left text-xs font-medium text-white/70">Log Index</th>
              <th class="px-3 py-2 text-left text-xs font-medium text-white/70">Event</th>
              <th class="px-3 py-2 text-left text-xs font-medium text-white/70">Address</th>
          ...
          <tbody class="divide-y divide-border">
            {#each paginatedLogs as log}
              <tr class="hover:bg-muted/30 transition-colors">
                ...
                <td class="px-3 py-2 whitespace-nowrap">
                  <span class="font-mono text-xs text-white/70">{log.logIndex}</span>
                </td>
                <td class="px-3 py-2 whitespace-nowrap">
                ...
                <td class="px-3 py-2 whitespace-nowrap">
                  <a 
                    href={`/blockexplorer/address/${log.address}`}
                    class="font-mono text-xs text-[var(--luke-primary)] hover:underline"
                  >
                    {formatTopic(log.address)}
                  </a>
                </td>
                <td class="px-3 py-2 whitespace-nowrap">
    ...
    {/if}
  {:else}
    <Card class="border-border bg-card/95 p-4">
      <p class="text-center text-sm text-white/70">No logs found for this transaction.</p>
    </Card>
  {/if}
</div>
