...
function getArtifactOfContract(contractName) {
  // Try direct path first: out/ContractName.sol/ContractName.json
  let current_path_to_artifacts = join(
    __dirname,
    "..",
  ...
  if (existsSync(current_path_to_artifacts)) {
    ...
    if (existsSync(artifactPath)) {
      const artifactJson = JSON.parse(readFileSync(artifactPath));
      return artifactJson;
    }
  }

  // Try to find the contract in all out directories (for cases like LUKE in luke.sol)
...
function main() {
  ...
  // When the broadcast contract is a proxy (e.g. ERC1967Proxy) but deployment name is the logic contract (e.g. LukeMain),
  // use the logic contract's ABI so the frontend reads/writes through the proxy correctly.
  Object.entries(allGeneratedContracts).forEach(([chainId, contracts]) => {
    Object.entries(contracts).forEach(([contractName, contractData]) => {
      ...
      if (deployedName) {
        const implArtifact = getArtifactOfContract(deployedName);
        const abi = implArtifact?.abi ?? contractData.abi;
        const inheritedFunctions = implArtifact
          ? getInheritedFunctions(implArtifact)
          : contractData.inheritedFunctions;
  ...
  // For chains that have deployments/*.json (e.g. 31337 local), overlay addresses from that file
  // so the frontend always gets the latest deployed addresses (broadcast may be stale or run-latest excluded).
  Object.keys(deployments).forEach((chainId) => {
    const raw = deployments[chainId];
    if (!raw || typeof raw !== "object") return;
    if (!allGeneratedContracts[chainId]) allGeneratedContracts[chainId] = {};
    Object.entries(raw).forEach(([address, name]) => {
      if (!name || name === "networkName") return;
  ...
  console.log(
    `📝 Updated TypeScript contract definition file on ${SVELTEKIT_TARGET_DIR}deployedContracts.ts`
  );
}
