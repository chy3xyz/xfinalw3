...
// Use DeployLocal.s.sol for localhost by default (more reliable)
let fileName = "DeployLocal.s.sol";
...
for (let i = 0; i < args.length; i++) {
  if (args[i] === "--network" && args[i + 1]) {
    ...
    fileName = args[i + 1];
    i++; // Skip next arg since we used it
  } else if (args[i] === "--use-mainnet-script") {
    // Allow using Deploy.s.sol for mainnet deployments
